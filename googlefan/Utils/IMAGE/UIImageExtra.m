...
+ (UIImage *)richImageNamed:(NSString *)name;
{
    if ([[UIDevice currentDevice] isLongiPhone]) {
        NSString *insertStr = @"-568h";
        if ([[UIDevice currentDevice] isiPhone6]) {
            insertStr = @"-375h";
        } else if ([[UIDevice currentDevice] isiPhone6p]) {
            insertStr = @"-414h";
        }
        NSMutableString *imageNameMutable = [name mutableCopy];
        NSRange dot = [name rangeOfString:@"."];
        if (dot.location != NSNotFound) {
            [imageNameMutable insertString:insertStr atIndex:dot.location];
        } else {
...
- (UIImage *)cropImage:(UIImage *)originalImage size:(CGSize)size {
    ...
    if (desiredSize.width > desiredSize.height * rate) {
        desiredSize.width = fabs(desiredSize.height * rate);
        cropRect = CGRectMake(0.0, 0.0, desiredSize.width, desiredSize.height);
        cropRect.origin.x = fabs((originalSize.width - desiredSize.width) / 2);
    }
    else if (desiredSize.height > desiredSize.width / rate) {
        desiredSize.height = fabs(desiredSize.width / rate);
        cropRect = CGRectMake(0.0, 0.0, desiredSize.width, desiredSize.height);
        cropRect.origin.y = fabs((originalSize.height - desiredSize.height) / 2);
    }
    else {
        cropRect = CGRectMake(0.0, 0.0, desiredSize.width, desiredSize.height);
    }
...
+ (UIImage *)transformImage:(UIImage *)tmpImage
{
