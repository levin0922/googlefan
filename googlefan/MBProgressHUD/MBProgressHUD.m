...
#define LABELFONTSIZE_IPHONE 14.0f
#define LABELDETAILSFONTSIZE 16.0f
#define LABELDETAILSFONTSIZE_IPHONE 14.0f
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	BOOL removed = NO;
	for (MBProgressHUD *v in [[view subviews] copy]) {
		if ([v isKindOfClass:[MBProgressHUD class]]) {
            v.removeFromSuperViewOnHide = YES;
            [v hide:animated];
            removed = YES;
        }
	}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.opacity = 0.8f;
        self.labelFont = [UIFont boldSystemFontOfSize:(IS_IPAD ? LABELFONTSIZE : LABELFONTSIZE_IPHONE)];
        self.detailsLabelFont = [UIFont boldSystemFontOfSize:(IS_IPAD ? LABELDETAILSFONTSIZE : LABELDETAILSFONTSIZE_IPHONE)];
        self.xOffset = 0.0f;
        self.yOffset = 0.0f;
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        if (self.labelText.length > 0) {
            ...
            dims = [self.labelText boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options: NSStringDrawingTruncatesLastVisibleLine |NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
//            dims = [self.labelText sizeWithFont:self.labelFont];
        ...
        if (nil != self.detailsLabelText) {
			...
			CGSize labelSize = [detailsLabel sizeThatFits:CGSizeMake(frame.size.width - 4*margin, maxHeight)];
